...
//			LET OP ! assumption that local writes are implicitely ordered by timestamp
			lastLW = op;
		}
	}
	...
	public synchronized Operation dequeue(Long maxTs) {
		...
		if(ret.getTS().compareTo(maxTs) > 0) return null;
	...
	public synchronized boolean contains(Operation op) {
		...
		/*
		//DEBUG
		System.out.println("Queue of size " + queue.size() +" is checked for containing op with " +
				"timestamp " + op.getTS());
		*/
