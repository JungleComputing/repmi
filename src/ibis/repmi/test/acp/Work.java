...
class Work extends UnicastRemoteObject implements i_Work {
    ...
    final int IDLE = 0, WORKING = 1, BLOCKED = 2, READY = 3, NEW = 4, ANY = 5;

    int slaves;

    int getWork = 0;

    Work(int vars, int slaves, int numVariables, int numValues,
            int numConnections, int numRelations, int numRelationPairs, int seed)
    ...
    public synchronized void vote(int var, boolean vote) throws RemoteException {
        work[var] = vote;
    }

    public synchronized void announce(int var) throws RemoteException {
    ...
    public synchronized void ready(int cpu) throws RemoteException {
